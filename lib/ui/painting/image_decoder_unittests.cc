...
class TestIOManager final : public IOManager {
 public:
  TestIOManager(fml::RefPtr<fml::TaskRunner> task_runner)
      : gl_context_(gl_surface_.CreateContext()),
        weak_gl_context_factory_(gl_context_.get()),
  ...
  fml::WeakPtr<GrContext> GetResourceContext() const override {
    return weak_gl_context_factory_.GetWeakPtr();
  }
 ...
 private:
  ...
  fml::WeakPtrFactory<GrContext> weak_gl_context_factory_;
...
TEST_F(ImageDecoderFixtureTest, ValidImageResultsInSuccess) {
  ...
  auto decode_image = [&]() {
    ...
    ASSERT_TRUE(image_decoder->IsValid());
