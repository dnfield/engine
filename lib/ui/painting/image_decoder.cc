...
ImageDecoder::ImageDecoder(
  ...
  // Cannot check the validity of the IOManager because we may not be on the IO
  // thread.
...
static SkISize ResizedDimensions(SkISize current_size,
...
static sk_sp<SkImage> ResizeRasterImage(sk_sp<SkImage> image,
  ...
  const auto resized_dimensions =
      ResizedDimensions(image->dimensions(), target_width, target_height);
